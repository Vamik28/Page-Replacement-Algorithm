//time complexity O(noframes * inputstring length * inputstring length);
function addTable() 
{
    ...
    for(i = 0;i<inputString.length;i++){ 
         if(i == 0)
         {
             ...
             for(j = 1;j<noOfFrames;j++)
                 ...
                 tables[j][i] = -1;                 //filling the table till all the frames are fill
             ...
             else
             {
                   ...
                   for(j = 1;j<noOfFrames + 1;j++)
                   {
                    if(tables[j][i] == inputString[i] - '0')
                          y = 1;  // check for hit and miss
                   }// Big O (noofFrames)
               if(y != 1){
                   var arr = new Array(noOfFrames);  // creating a new array arr
                   var l,o,count = 0; // declaring variables
                   for( l = 1;l<noOfFrames  + 1;l++)   //time complexity O(noOFFrames)
                    arr[l - 1] = tables[l][i];   // assign value of ith column to array
               
               for(l = 0;l<noOfFrames;l++){
                   for( o = i + 1;o<inputString.length;o++){
                       if(arr[l] == inputString[o] - '0'){
                           arr[l] = o;                    //assigning their order in which they occur
                           break;                        
                         }
                   }//time complexity O(noOfFrames * length of inputString)
                   if(o == inputString.length)
                          arr[l] = -1;                // if the elemnt not occur again we assign -1
                   
               }
               var replace = 0;     // declaring a variable replace
               for(l = 0;l<noOfFrames;l++){
                    if(arr[l] == -1){          //if element will not occur again we replace that one and break the loop
                         replace = l;
                         break;
                    }
                    if(arr[l]>arr[replace])
                        replace = l;             // finding the element which occur last
                    
                }//time complexity O(noOFFrames)
               replace++;
               tables[replace][i] = inputString[i] - '0';
               tables[noOfFrames + 1][i] = replace*-1; 
                   
               }
               else
               var f,b;
               for(f = 1;f<=noOfFrames;f++){
                 if(tables[f][i - 1]== inputString[i]){
                    b = f;
                    break;
                 }
               }
              if(f != noOfFrames + 1)
               tables[noOfFrames + 1][i] = b;   // for a hit case
                           
              }
         }
    }
      // time complexity for creating table is O(noframes * inputstring length * inputstring length);
        ...
        for (var row = 0; row < noOfFrames + 2; row++) 
        {
          ...
          for (var column = -1; column < inputString.length; column++) 
          {
            ...
            } else if (row == noOfFrames + 1) 
            {
              ...
              else if(tables[noOfFrames + 1][column] != -1 && row == tables[noOfFrames + 1][column]  )
